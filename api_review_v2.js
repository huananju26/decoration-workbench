...
routerAdd('POST', '/api/request/submit', (e) => {
  ...
  /* 多团队（2026-08-31）：原来这里是 `if (auth.get('org_id')) throw '你已属于某个公司'`。
     现已允许一个人属于多个团队，所以不再按「有没有公司」一刀切拦，
     改为在 join 分支里精确判断「是否已经是【该】团队的活跃成员」。 */
  ...
  if (data.type === 'join') {
    ...
    if (inv.get('revoked')) throw new BadRequestError('该邀请码已被管理员取消');
    const exp = String(inv.get('expires') || '').replace(' ', 'T');
    if (exp && new Date(exp) < new Date()) throw new BadRequestError('邀请码已过期');
    const invOrg = String(inv.get('org_id') || '');
    /* 精确判重：只拦「已经是该团队活跃成员」，不拦「已属于别的团队」 */
    let already = null;
    try {
      already = $app.findFirstRecordByFilter('memberships',
        "user_id = '" + auth.id + "' && org_id = '" + invOrg + "' && status = 'active'");
    } catch (errM) { already = null; }
    if (already) throw new BadRequestError('你已经是该团队的成员了');
    rec.set('org_id', invOrg);
...
routerAdd('POST', '/api/request/review', (e) => {
  ...
  /* 多团队（2026-08-31）：原来是 `if (target.get('org_id')) throw '该用户已属于某个公司'`，
     评审路径上的第二道单团队拦截。改为精确判「是否已是【该】团队的活跃成员」。 */
  if (typ !== 'org') {
    const wantOrg = String(rec.get('org_id') || '');
    let dupMem = null;
    try {
      dupMem = $app.findFirstRecordByFilter('memberships',
        "user_id = '" + uid + "' && org_id = '" + wantOrg + "' && status = 'active'");
    } catch (errM) { dupMem = null; }
    if (dupMem) throw new BadRequestError('该用户已经是本团队成员');
  }

  let orgId = rec.get('org_id');
  let grantRole = 'member';
  if (typ === 'org') {
    ...
    grantRole = String(rec.get('role') || 'member');
  }

  /* 成员关系（唯一权威）：同一 (user, org) 复用同一行 —— 之前退出/被移出的把 status 翻回 active */
  try {
    let memRow = null;
    try {
      memRow = $app.findFirstRecordByFilter('memberships',
        "user_id = '" + uid + "' && org_id = '" + orgId + "'");
    } catch (e1) { memRow = null; }
    if (memRow) {
      memRow.set('status', 'active');
      memRow.set('role', grantRole);
  ...
  /* users.org_id / users.role 只是「当前激活团队」缓存：
     原本没有激活团队才写入；已在别的团队里就不夺走，前端提示「去切换团队」。 */
  let switched = false;
  if (!target.get('org_id')) {
    target.set('org_id', orgId);
    target.set('role', grantRole);
    $app.save(target);
    switched = true;
  }
  ...
  return e.json(200, { ok: true, status: 'approved', orgId: orgId, role: grantRole, switched: switched });
});
