...
routerAdd('POST', '/api/admin/scan', (e) => {
  ...
  for (let i = 0; i < orows.length; i++) {
    ...
    const reasons = [];
    let level = 'safe';
    const hasPaid = (paid > 0 || subs > 0);

    // 只有「付过钱」才是真禁止：force 也删不掉，前端连勾都不给勾。
    // 「有成员」不是禁止 —— 没付过钱的公司照样能删，只是要求 force（先把成员踢回无公司）。
    // 早期版本把「有成员」也判成 block，结果前端 disabled 掉复选框，
    // 「强制删除」开关成了摆设（后端支持、前端堵死），已修正。
    if (hasPaid) {
      level = 'block';
      reasons.push('有付费/订阅记录');
      if (members > 0) reasons.push('有 ' + members + ' 名成员');
    } else if (members > 0) {
      level = 'warn';
      reasons.push('有 ' + members + ' 名成员，删除需勾选「强制删除」（成员会被踢回无公司状态）');
    } else if (units > 0 || bytes > 0) {
      level = 'warn';
...
routerAdd('POST', '/api/admin/purge', (e) => {
  ...
  const data = new DynamicModel({ user_ids: '', org_ids: '', force: false, purge_paid: false });
  e.bindBody(data);
  ...
  for (let i = 0; i < oids.length; i++) {
    ...
    // 付费过的默认不删 —— 这是财务底线；只有勾选 purge_paid（平台管理员受控覆盖）才放行
    let paid = 0;
    try {
    ...
    if (paid > 0 || subs > 0) {
      if (!data.purge_paid) {
        skipped.push({ id: id, name: name, reason: '有付费/订阅记录，需勾选「允许删除付费公司」' });
        continue;
      }
      // 已显式勾选 purge_paid：放行删除；注意成员仍需先踢回「无公司」状态（见下方）
    }

    // 有成员：默认拦下；force 才删，且先把成员踢回「无公司」状态
    let mrows = [];
    try { mrows = $app.findRecordsByFilter('users', 'org_id = "' + id + '"', '-created', 500, 0); } catch (err) { mrows = []; }
    if (mrows.length && !data.force && !data.purge_paid) {
      skipped.push({ id: id, name: name, reason: '有 ' + mrows.length + ' 名成员，需勾选「强制删除」或「允许删除付费公司」' });
      continue;
    }
